...
import { PlusCircleIcon, XMarkIcon, CheckCircleIcon, PencilSquareIcon, TrashIcon } from "@heroicons/react/24/outline";
import { useParams } from "react-router-dom";
import { getProjects, getProject, toggleTaskState, deleteTask } from "../api/projectService";
...
export default function Project() {
  ...
  return (
    <div className="space-y-10 p-6">
        ...
        <>
          <h2 className="text-2xl sm:text-3xl font-medium mb-4">{proyectoSeleccionado.name}</h2>
        ...
        {proyectoSeleccionado && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                {tasks.map(task => (
                  <div key={task._id} className="bg-white/90 p-4 rounded-2xl shadow-md flex flex-col justify-between transition transform hover:scale-105">
                    <div className="flex flex-col flex-1 mb-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={task.status === "completada" ? "line-through font-semibold" : "font-semibold"}>
                          {task.title}
                        </span>
                      </div>
                    ...
                    <div className="flex space-x-2 mt-2">
                      <button onClick={() => openEditModal(task)} className="p-2 bg-yellow-400 hover:bg-yellow-500 rounded-lg transition">
                        <PencilSquareIcon className="w-4 h-4 text-white" />
                      </button>
                      <button onClick={() => eliminarTarea(task._id)} className="p-2 bg-red-500 hover:bg-red-600 rounded-lg transition">
                        <TrashIcon className="w-4 h-4 text-white" />
                      </button>
                      <button
                        onClick={() => toggleEstado(task._id)}
                        className={`px-3 py-1 rounded-lg text-white font-medium transition ${task.status === "completada" ? "bg-green-400 hover:bg-green-500" : "bg-yellow-400 hover:bg-yellow-500"}`}
                      >
        ...
        {(showCreateModal || showEditModal) && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-xl font-semibold mb-4 text-gray-800">{showCreateModal ? "Nueva Tarea" : "Editar Tarea"}</h3>
              ...
              <div className="flex justify-end mt-5 space-x-2">
                <button onClick={closeModal} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 transition">
                  Cancelar
                </button>
                <button
